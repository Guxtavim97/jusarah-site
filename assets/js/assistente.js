/* ============================================================
   Assistente de IA da Jusarah — painel de conversa com voz.
   ...
   ============================================================ */
(function(){
  var A = { sb:null, url:'', nome:'Assistente', contexto:'geral', falar:true, ouvindo:false, rec:null, pensando:false };
  ...
  + '.ia-fab .dot{position:absolute;top:12px;right:12px;width:9px;height:9px;border-radius:50%;background:#ffc44d;display:none}'
  ...
  + '.ia-ft{display:flex;align-items:center;gap:8px;padding:11px;background:#0d1f36;border-top:1px solid rgba(127,212,255,.12)}'
  + '.ia-ft input{flex:1;background:#0b1a2e;border:1px solid rgba(127,212,255,.2);border-radius:11px;'
  +   'padding:11px 13px;color:#eaf2fb;font-size:.92rem;font-family:inherit}'
  + '.ia-ft input:focus{outline:none;border-color:#4a9fd4}'
  ...
  var SUGEST = {
    ...
    agro:[
      ...
      'Quais clientes pediram orçamento e ainda não receberam proposta?'
    ],
    geral:['O que você consegue me dizer sobre o sistema?']
  };

  var wrap, body, input, micBtn, spkBtn, fab;
  ...
  function injeta(){
    ...
    fab = el('button','ia-fab','✨<span class="dot"></span>');
    ...
    wrap.innerHTML =
      ...
      '<div class="ia-ft">'+
        ...
        '<input type="text" placeholder="Pergunte ou toque no microfone…" autocomplete="off">'+
        '<button class="ia-send" title="Enviar">➤</button>'+
      '</div>';
    document.body.appendChild(wrap);

    body   = wrap.querySelector('.ia-body');
    input  = wrap.querySelector('.ia-ft input');
  ...
  function abre(){
    wrap.classList.add('open'); fab.querySelector('.dot').style.display='none';
  ...
  function saudacao(){
    ...
    addMsg('a', 'Oi! Sou a assistente da Jusarah. Pode me perguntar por texto ou falando no microfone. 🎤');
  }

  function addMsg(tipo, texto, fonte){
    var m = el('div','ia-msg '+tipo, esc(texto));
  ...
  async function pergunta(q){
    ...
    try{
      ...
      var resp = await fetch(A.url + '/functions/v1/assistente', {
        ...
        body: JSON.stringify({ pergunta:q, contexto:A.contexto })
      });
      ...
      var m = addMsg('a', data.resposta || '(sem resposta)', data.fonte);
  ...
  function toggleMic(){
    ...
    A.rec.onresult = function(e){
      var t = '';
      for(var i=e.resultIndex;i<e.results.length;i++){ t += e.results[i][0].transcript; if(e.results[i].isFinal) finalTxt += e.results[i][0].transcript; }
      input.value = t;
    };
    A.rec.onerror = function(){ pararMic(); };
    A.rec.onend = function(){
      micBtn.classList.remove('rec'); A.ouvindo=false;
      if((input.value||'').trim()) enviar();
    };
